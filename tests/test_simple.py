import numpy as np
from src.eigen import solve_eigen as solve_simple
from src.eigen_BC import solve_eigen as solve_bc

def test_small_grid():
    vals, _ = solve_simple(N=5, potential='well', n_eigs=3)
    assert len(vals) == 3
...
def test_bc_matrix_size():
    ...
    N = 5
    vals, vecs = solve_bc(N=N, potential='well', n_eigs=3, bc='dirichlet')
    assert vecs.shape[0] == (N-2)**2
    assert len(vals) == 3

def test_vecs_normalized():
    # Check if the ground state is normalized (L2 norm = 1)
    _, vecs = solve_bc(N=5, potential='well', n_eigs=1, bc='dirichlet')
    gs = vecs[:, 0]
